...
class PrintQueueDriver {
	 ...
	 */
	public enum Command {
		//  : add new Command enum lprd
		lpr, lpq, Lprd, lprmid, lprmactive, lprmall, quit
	}
	 ...
	 */
	static void lpr() throws IOException {
	 ...
	 */
	static void lpq() throws IOException {
		myQ.lpq(pw);
		pw.flush();
	}
	 ...
	 * 
	 * @throws IOException            If there is an i/o error in reading the name
	 *                                and id.
	 * @throws NoSuchElementException If there is no matching ob in the print queue.
	 */
	static void lprmid() throws IOException {
		System.out.print("Job Id? => ");
		System.out.flush();
		int id = Integer.parseInt(stdbr.readLine());
	 ...
	 * 
	 * @throws IOException            If there is an error in reading the owner
	 *                                name.
	 * @throws NoSuchElementException If there is no matching job, or if this queue
	 *                                is empty.
	 */
	static void lprmall() throws IOException {
		System.out.print("Owner Name? => ");
		System.out.flush();
		String owner = stdbr.readLine();
	 ...
	 * @throws NoSuchElementException If a job is sought that does not exist in the
	 *                                print queue.
	 */
