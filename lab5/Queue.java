...
public class Queue<T> {
	...
	public void enqueue(T item) {
		//  1: check if the item is null and if it is throw an
		// IllegalArgumentException.
		if( item == null ) {
			throw new IllegalArgumentException("Item is null");
			} else {
			list.add(item);	// : add the item to the data list.
			}
	 ...
	 */
	public T dequeue() {
		// : check if the data list is empty and if it is throw a
		// NoSuchElementException.
		if( isEmpty() ) {
			throw new NoSuchElementException("The list is empty");
		}

		// : declare a variable of type T named first and initialize it to element
		// 0 of the data list
		T first = list.get(0);

		// : remove element 0 from the data list
		list.remove(first);
