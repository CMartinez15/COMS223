...
public class PostfixEvaluator {
	...
	public float runSteps(PrintWriter pw) throws IllegalExpressionException {
		while (exprTok.hasMoreTokens()) {
			//  within the while loop, call runSome with the parameters 1 and pw
			// which forces a single step to be executed.
			runSome(1, pw);

			//  within the while loop, after the call the runSome, add a call to
			// printStatus passing pw.
			printStatus(pw);

			//  within the while loop, after the call the printStatus, call the
			// flush method on pw to force output to the console.
			pw.flush();

		}

		// add an if statement to check if size of postStack is greater than
		...
		// replace the "return 0;" statement with a return statement that
