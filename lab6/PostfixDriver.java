...
public class PostfixDriver {
	 ...
	 *
	 */
	// : add new runsteps command to the Command enum
	public enum Command {
		runall, runsome, runsteps, restart, quit
	}
	...
	public static void main(String[] args) throws IOException, IllegalExpressionException {
		...
		while ((cmd = getCommand()) != Command.quit) {
			...
			case restart:
				startEval();
				break;
			// : add case for runsteps command, call runSteps method on myEvaluator
			// instance passing pw as parameter.
			case runsteps:
				myEvaluator.runSteps(pw);
				break;
			default:
				break;
			}
